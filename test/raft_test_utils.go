...
func printLog(updateOperation []*surfstore.UpdateOperation) {
	fmt.Println("----------")
	fmt.Println("log printing")
	for idx, log := range updateOperation {
		fmt.Println("idx: ", idx, ", log.Term:", log.Term, ", filename: ", log.FileMetaData.Filename)
	}
	fmt.Println("finish log printing")
	fmt.Println("----------")
}
...
func CheckInternalState(isLeader *bool, term *int64, log []*surfstore.UpdateOperation, fileMetaMap map[string]*surfstore.FileMetaData, server surfstore.RaftSurfstoreClient, ctx context.Context) (bool, error) {
	...
	// fmt.Println("Hope it works here")
	if err != nil {
