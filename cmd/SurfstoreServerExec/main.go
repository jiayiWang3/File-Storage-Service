...
func main() {
	...
	// }
	// TODO: changed to get blockStoreAddrs
	args := flag.Args()
...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	// panic("todo")
	for _, str := range blockStoreAddrs {
		fmt.Println(str)
	}
	grpcServer := grpc.NewServer()
	if strings.Compare(serviceType, "meta") == 0 {
		metaServ := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, metaServ)
	} else if strings.Compare(serviceType, "block") == 0 {
		blockServ := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, blockServ)
	} else if strings.Compare(serviceType, "both") == 0 {
		metaServ := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, metaServ)
		blockServ := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, blockServ)
